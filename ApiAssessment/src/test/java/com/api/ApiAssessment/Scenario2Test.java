...
public class Scenario2Test
{
   ...
   public static void ArchivingTest() //String ChannelToArchive
   {
	   	Properties prop = RestUtils.filename();	
       try
       {
    	   boolean archiveChannel =  HelperMethods.JoinAndArchiveChannel(prop.getProperty("Auth_Token"),"sample53");
	       if(archiveChannel)	
	       {
	    	   //Assert statement to check if the channel is archived
		       	Assert.assertTrue(archiveChannel);
		       	
		       	boolean isChannelArchivingSuccessful = HelperMethods.isChannelArchived(prop.getProperty("Auth_Token"),"sample53");
